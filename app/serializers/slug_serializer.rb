    ...
    def collection_as_array
      ...
      if slugs_map.present?
        slugs_map.keys.each do |key|
          slugs_collection << ["<a href= #{slugs_map[key][:long_url]} target='_blank'>#{key}</a>".html_safe, slugs_map[key][:long_url], slugs_map[key][:sanitized_url], slugs_map[key][:created_at].strftime("%d %b, %Y")]
        end
      end
